        ...
        if (rows.length >= 1) {
            for (var i = 0, len = rows.length; i < len; i++) {
                if (rows[i][0] == idservicio) {
                    var newRow = [idservicio, servicio, parseInt(txtCantidad) + parseInt(rows[i][2]), lblPrecioServicio, parseInt(lblTotalServicio) + parseInt(rows[i][4]), rows[i][5]];
...
$('#addCita').click(function () {
    ...
    rowsS.toArray();
    rowsM.toArray();
    var idCita = $('#hdf_Id_Cita')[0].value;

    $.ajax({
        type: "POST",
        url: "/Cita/Detalles?rowsS=" + setJsonS(rowsS.toArray(), idCita) + "&&rowsM=" + setJsonM(rowsM.toArray(), idCita),
        contentType: 'application/json; charset=utf-8',
        success: function (response) {
            debugger
            if (response == "ok") {
